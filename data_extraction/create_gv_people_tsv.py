...
OUTFILE='%s/tabular_gv%s_data.tsv' % (OUTDIR, which_data)
...
if which_data=='americans':
	pickle_with_all_data='%s/gv_americans.p' % TMPDIR
	all_people, american_ids=utils.extract_americans(americans_nt)
	print('Americans data loaded. %d entries' % len(all_people))
else:
